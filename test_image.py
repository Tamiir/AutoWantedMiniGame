...
template_image = np.array(pyautogui.screenshot(region=(leftt, topt, widtht, heightt)))
...
result1 = cv2.matchTemplate(data1_gray, template_gray, cv2.TM_CCORR_NORMED)
result2 = cv2.matchTemplate(data2_gray, template_gray, cv2.TM_CCORR_NORMED)
result3 = cv2.matchTemplate(data3_gray, template_gray, cv2.TM_CCORR_NORMED)
result4 = cv2.matchTemplate(data4_gray, template_gray, cv2.TM_CCORR_NORMED)
...
bigger_gray = cv2.cvtColor(bigger_image, cv2.COLOR_BGR2GRAY)
template_gray = cv2.cvtColor(template_image, cv2.COLOR_BGR2GRAY)
